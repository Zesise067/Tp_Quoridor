...
//		wall = new int[] {6,6,6,0}; //------------------------------------------------------------ 測試用
		...
		coordinateYX = new int[5][2]; //座標 YX[玩家][座標] 。[0][0] 進攻方 。[1~4(北南西東)]。[0] Y 。[1] X
		for(int i = 0;i<coordinateYX.length;i++) Arrays.fill(coordinateYX[i], 0); //初始化
...
//		frame.getContentPane().add(this); //----------------------------------------------------------------------- 測試用
		...
		Quoridor.GameRules(infCheckerboard, coordinateYX);
		infCheckerboard = Quoridor.getInfCheckerboard();
		printCheckerboard();
	}
